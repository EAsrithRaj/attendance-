  ...
  return (
    ...
    >
      <div className="animate-fade-in space-y-4">
        ...
        {progress.total > 0 && (
          <div className="rounded-2xl border border-border bg-card px-4 py-3 shadow-sm space-y-2">
            <div className="flex justify-between text-xs text-muted-foreground font-mono">
              <span className="font-semibold uppercase tracking-wide">Semester</span>
        ...
        {/* Bunk budget */}
        {bunkStats.length > 0 && (
          <div className="rounded-2xl border border-border bg-card p-4 shadow-md hover:shadow-lg transition-all duration-200 space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Bunk Budget</p>
                ...
                return (
                  <div key={s.id} className="space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium text-card-foreground truncate max-w-[55%]">{s.name}</span>
                      <span className="text-xs font-mono text-muted-foreground shrink-0">
                        {s.percentage.toFixed(1)}%{" "}
                        {s.bunkBuffer > 0 ? (
                          <span className="text-attendance-green font-semibold">· skip {s.bunkBuffer}</span>
                        ) : s.mustAttendNext > 0 ? (
                          <span className="text-attendance-red font-semibold">· attend {s.mustAttendNext}</span>
                        ) : null}
                      </span>
                    </div>
        ...
        <div className="flex items-center justify-between px-1">
          <Button variant="ghost" size="sm" onClick={prev} className="h-9 w-9 p-0">
            <ChevronLeft className="h-5 w-5" />
          </Button>
          <span className="font-semibold text-base text-foreground">{monthName}</span>
          <Button variant="ghost" size="sm" onClick={next} className="h-9 w-9 p-0">
              ...
              <Tooltip key={dateStr}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={`relative aspect-square flex items-center justify-center rounded-2xl text-xs font-mono font-medium transition-all duration-150 ${CELL_COLORS[info.state]} ${
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center pt-1">
          {(([["GREEN","Safe / All present"],["YELLOW","Warning / Near limit"],["RED","Danger / Below min"],["GREY","Off / Holiday"],["BLUE","Exam period"]] as const)).map(([st, label]) => (
            ...
            return hasContent ? (
              <div className="space-y-3">
                    ...
                    <div key={slot.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-bold text-base text-card-foreground">{sub.name}</span>
                            {slot.weight === 3 && <Badge variant="secondary" className="text-[10px]">LAB</Badge>}
                            {rec && (
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                                rec.status === "PRESENT" ? "bg-attendance-green/15 text-attendance-green"
                                : rec.status === "ABSENT" ? "bg-attendance-red/15 text-attendance-red"
                                : "bg-muted text-muted-foreground"
                              }`}>
                                {rec.status === "PRESENT" && <Check className="inline h-3 w-3 mr-1 mb-0.5" />}
                                {rec.status === "ABSENT" && <X className="inline h-3 w-3 mr-1 mb-0.5" />}
                                {rec.status === "CANCELLED" && <Ban className="inline h-3 w-3 mr-1 mb-0.5" />}
                        ...
                        {rec && (
                          <button
                            onClick={() => handleClear(slot.subjectId, slot.id)}
                            className="ml-2 shrink-0 p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors duration-150"
                            title="Clear mark"
                          >
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <Button
                          ...
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec?.status === "PRESENT" ? STATUS_STYLES.PRESENT : ""}`}
                          onClick={() => handleMark(slot.subjectId, slot.id, slot.weight, "PRESENT")}
                        >
                          <Check className="h-4 w-4" />Present
                        </Button>
                        <Button
                          size="sm"
                          variant={rec?.status === "ABSENT" ? "default" : "outline"}
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec?.status === "ABSENT" ? STATUS_STYLES.ABSENT : ""}`}
                          onClick={() => handleMark(slot.subjectId, slot.id, slot.weight, "ABSENT")}
                        >
                          <X className="h-4 w-4" />Absent
                        </Button>
                        <Button
                          size="sm"
                          variant={rec?.status === "CANCELLED" ? "default" : "outline"}
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec?.status === "CANCELLED" ? STATUS_STYLES.CANCELLED : ""}`}
                          onClick={() => handleMark(slot.subjectId, slot.id, slot.weight, "CANCELLED")}
                        >
                  ...
                  return (
                    <div key={rec.slotId} className="rounded-2xl border border-border bg-card p-4 shadow-sm space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        <button
                          onClick={() => deleteExtraClass(rec.slotId, rec.date)}
                          className="ml-2 shrink-0 p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors duration-150"
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <Button
                          ...
                          variant={rec.status === "PRESENT" ? "default" : "outline"}
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec.status === "PRESENT" ? STATUS_STYLES.PRESENT : ""}`}
                          onClick={() => handleMark(rec.subjectId, rec.slotId, rec.weightSnapshot, "PRESENT")}
                        >
                          <Check className="h-4 w-4" />Present
                        </Button>
                        <Button
                          ...
                          variant={rec.status === "ABSENT" ? "default" : "outline"}
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec.status === "ABSENT" ? STATUS_STYLES.ABSENT : ""}`}
                          onClick={() => handleMark(rec.subjectId, rec.slotId, rec.weightSnapshot, "ABSENT")}
                        >
                          <X className="h-4 w-4" />Absent
                        </Button>
                        <Button
                          ...
                          variant={rec.status === "CANCELLED" ? "default" : "outline"}
                          className={`h-12 w-full text-sm font-medium gap-1.5 active:scale-95 transition-all duration-150 ${rec.status === "CANCELLED" ? STATUS_STYLES.CANCELLED : ""}`}
                          onClick={() => handleMark(rec.subjectId, rec.slotId, rec.weightSnapshot, "CANCELLED")}
                        >
                          <Ban className="h-4 w-4" />Cancel
                        </Button>
                      </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-2xl border border-dashed border-border p-3.5 text-sm font-medium text-muted-foreground hover:bg-muted/50 hover:text-foreground hover:border-muted-foreground transition-all duration-200 active:scale-95"
                >
              ...
              <div className="space-y-3">
                {!selectedHoliday && (
                  <p className="text-sm text-muted-foreground text-center py-4">No classes this day.</p>
                )}
                <button
                  onClick={() => setExtraModalOpen(true)}
                  className="flex items-center justify-center gap-2 w-full rounded-2xl border border-dashed border-border p-3.5 text-sm font-medium text-muted-foreground hover:bg-muted/50 hover:text-foreground hover:border-muted-foreground transition-all duration-200 active:scale-95"
          ...
          <div className="space-y-3 mt-4">
            ...
            {!selectedHoliday && selectedPred?.classPredictions
              ...
              .map((cp) => {
                ...
                const bg    = cp.skipState === "RED" ? "border-l-attendance-red" : cp.skipState === "YELLOW" ? "border-l-attendance-yellow" : "border-l-attendance-green";
                return (
                  <div key={cp.slotId} className={`flex items-center justify-between rounded-2xl border-l-4 border border-border bg-card p-3.5 shadow-sm ${bg}`}>
                    <div>
                    ...
                    <span className={`text-sm font-bold ${color} shrink-0 ml-3`}>{label}</span>
                  </div>
                );
              })}
